// GuideHeader: sticky top bar for the /guide variant. Same chrome as the
// main Header, different nav targets, plus an A/B switch back to `/`.
...
export function GuideHeader({ onNav }: GuideHeaderProps) {
  ...
  const links = [
    { id: 'vibe', label: 'vibe coded?' },
    { id: 'noise', label: 'the noise' },
    { id: 'guide', label: 'the guide' },
    { id: 'instincts', label: 'why us' },
  ];
  return (
      ...
      <div style={{
        ...
        <div style={{ maxWidth: 'var(--container-wide)', margin: '0 auto', padding: '0 var(--gutter)', height: 68, display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 16 }}>
          ...
          <nav style={{ display: 'flex', alignItems: 'center', gap: 4 }} className="bg-desktop-nav">
            ...
            <a href="/" style={{ fontFamily: 'var(--font-mono)', fontSize: 'var(--t-2xs)', color: 'var(--text-faint)', border: '1px solid var(--border-strong)', borderRadius: 'var(--r-1)', padding: '7px 11px', marginLeft: 8 }}
               title="Switch to variant A">
              variant b <span style={{ color: 'var(--accent)' }}>⇄ view a</span>
            </a>
            <Button variant="primary" prompt size="sm" style={{ marginLeft: 4 }} onClick={() => onNav('book')}>Talk to a guide</Button>
          </nav>
        ...
        {menu && (
          <div className="bg-mobile-nav" style={{ flexDirection: 'column', padding: '8px var(--gutter) 18px', borderTop: '1px solid var(--border-hairline)', background: 'var(--surface-base)' }}>
            {[...links, { id: 'book', label: 'talk to a guide' }].map(l => (
            ...
            <a href="/" style={{ fontFamily: 'var(--font-mono)', fontSize: 'var(--t-sm)', color: 'var(--text-faint)', padding: '14px 0' }}>⇄ view variant a</a>
          </div>
        )}
      </div>
    </header>
